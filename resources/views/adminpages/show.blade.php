...
@section('title', ucfirst($post->name))

@section('pageview')
<h1>
    Control board
    <small>{{ ucfirst($post->name)."'s acount" }}</small>
  </h1>
  <ol class="breadcrumb">
    <li><a href="#"><i class="fa fa-dashboard"></i> Control</a></li>
    <li class="active">{{ ucfirst($post->name)."'s acount" }}</li>
  </ol>
@endsection
    ...
    <div class="col-md-6 col-md-offset-3">
        ...
        <div class="box box-widget widget-user-2">
            ...
            <div class="widget-user-header bg-green">
                ...
                <h5 class="widget-user-desc">{{ ucfirst($post->job_title) }}</h5>
            </div>
            <div class="box-footer no-padding">
                <ul class="nav nav-stacked">
                  <li><a href="#">Number of books <span class="pull-right badge bg-green">{{ count($books) }} books</span></a></li>
                  ...
                  @if(Auth::user()->job_title == 'admin')
                    <li><a href="{{ route('admin.admins.edit', $post->id) }}">Edit {{ ucfirst($post->name)."'s acount" }}<span class="pull-right badge bg-aqua">Edit</span></a></li>
                    <li><a href="{{ route('admin.admins.destroy', $post->id) }}"onclick="event.preventDefault();
                        document.getElementById('logout-form').submit();">Delete {{ ucfirst($post->name)."'s acount" }}<span class="pull-right badge bg-red">Delete</span></a>
        ...
        <div class="box box-danger collapsed-box">
            <div class="box-header with-border">
                <h3 class="box-title">{{ ucfirst($post->name)."'s books" }}</h3>
